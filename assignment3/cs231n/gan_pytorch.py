...
def discriminator_loss(logits_real, logits_fake):
    ...
    labels1 = torch.ones_like(logits_real).type(dtype).squeeze()
    labels0 = torch.zeros_like(logits_fake).type(dtype).squeeze()
    loss = bce_loss(logits_real, labels1) + bce_loss(logits_fake, labels0)
...
def generator_loss(logits_fake):
    ...
    labels0 = torch.ones_like(logits_fake).type(dtype).squeeze()
    loss = bce_loss(logits_fake, labels0)
