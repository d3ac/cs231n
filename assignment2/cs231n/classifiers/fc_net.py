    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization == 'batchnorm':
                ...
                X, caches['layer' + str(i)] = affine_bn_relu_forward(X, W, b, gamma, beta, self.bn_params[i - 1])
            else:
                X, caches['layer' + str(i)] = affine_relu_forward(X, W, b)
            if self.use_dropout:
                X, caches['dropout' + str(i)] = dropout_forward(X, self.dropout_param)
        
        W = self.params['W' + str(self.num_layers)]
        b = self.params['b' + str(self.num_layers)]
        scores, caches['layer' + str(self.num_layers)] = affine_forward(X, W, b)
