...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        norm = (x - mean) / std
        out = gamma * norm + beta
        cache = (x, mean, var, std, norm, out, gamma, beta, eps)
...
def batchnorm_backward(dout, cache):
    ...
    #太麻烦了不想写
    x, x_mean, x_var, _, x_norm, _, gamma, _, eps = cache
    m = x.shape[0]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    out = gamma * norm + beta
    cache = (x, norm, mean, var, gamma, beta, G, eps)
